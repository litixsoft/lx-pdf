...
module.exports = function () {
     ...
     */
    function addTextToDocument(document, sectionName, sectionInfo) {
        // Get some informations
        var contentFormat = getValue('options.sections.' + sectionName, undefined); // Retruns undefined, if section not exits
        ...
        var text = sectionInfo.data,
            sectionDim = getSectionDimension(sectionName),
            ...
            pixelWidth = sectionDim.width * 2.0,
            result = {
                done: true,
                error: false,
                data: text,
        ...
        if (showTextboxes) {
            document.rect(sectionDim.left, sectionDim.top, sectionDim.width, sectionDim.height).stroke('lightsteelblue');
        }
        ...
        if (typeof text === 'string') {
            ...
            var maxAvailableLines = Math.floor(sectionDim.height / currentLineHeight); // Maximal available Lines in the current area

            // HINT: lineHeight * 2 is the minimum Height for a textline
            if (sectionDim.height < currentLineHeight) {
                throw new Error('The height of the content area (' + sectionName + ') is too small. Minmium ' + currentLineHeight + '. Given ' + sectionDim.height + '.');
            }
            ...
            var linePositionY = sectionDim.top + result.topIndex;
            lxHelpers.forEach(lines, function(text) {
                document.text(text, sectionDim.left, linePositionY, format);
        ...
        if (typeof text === 'object') {
            document.list(text, sectionDim.left,sectionDim.top, format);
        }
     ...
     * @returns {*}
     */
    function addTableToDocument(document, sectionName, sectionInfo, newPageCallback) {
        // Get some informations
        var contentFormat = getValue('options.sections.' + sectionName, undefined); // Returns undefined, if section not exits
        ...
        if (!sectionInfo.data) {
            console.log('WARNING: Table for Section "' + sectionName + '" contains no data.');
            return {done: true, error: true};
        }
        ...
        if (tableWidth > sectionDim.width) {
            return {done: true, error: true};
        }

        // some variables
//        var currentLineHeight = document.currentLineHeight(true);
        ...
        tableCellHeights.push(tableCurrentRowHeight);
        ...
        var topIndex = result.topIndex,
            currentCellPositionLeft = sectionDim.left,
            currentCellPositionTop = sectionDim.top + topIndex,
            currentContentHeight = sectionDim.height - topIndex,
        ...
        for (var idxRow = 0; idxRow < tableContentData.length; idxRow++) {
            ...
            lxHelpers.forEach(tableContentData[idxRow], function(cellData) {
                ...
                // Render cell
                document.text(cellData.text, currentCellPositionLeft, currentCellPositionTop, cellData.format);
                currentCellPositionLeft += cellData.format.width;
            });

            currentCellPositionLeft = sectionDim.left;
            currentCellPositionTop += tableCellHeights[idxRow];
            topIndex += tableCellHeights[idxRow];

            // Calculate the rest height
            if (idxRow < tableCellHeights.length) {
                currentContentHeight -= tableCellHeights[idxRow];
            }

            // If table contains a Header. Then draw a nice line. Sweet!
            if (drawHeader) {
                document.moveTo(sectionDim.left, currentCellPositionTop)
                    .lineTo(sectionDim.left + sectionDim.width, currentCellPositionTop)
                    .stroke();
            ...
            if (currentContentHeight < 0) {
                ...
                // Reset Positions
                currentCellPositionTop = sectionDim.top;
                currentContentHeight = sectionDim.height - topIndex;
            }
        }
        ...
        lxHelpers.forEach(documentData, function(sectionName, sectionData) {
            ...
            lxHelpers.forEach(sectionData, function(entry) {
                ...
                if (entry.text) {
                    ...
                    imageHeight = currentPDFDocument.image(entry.image.filename).y - imageHeight;
