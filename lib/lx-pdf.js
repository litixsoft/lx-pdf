...
module.exports = function () {
     ...
     * @returns {number}
     */
    function getTextHeight(document, text, maxWidth) {
        if (!text) {
            return 0;
        }
...
//        lxHelpers.forEach(textList, function(item) {
//            var textWidth = Math.ceil(document.widthOfString(item));
//
//            if ()
//        });
...
//        return parseInt(textItem, 10) + 1 + parseInt(wrapLines, 10);
    ...
    function addTextToDocument(document, sectionName, sectionInfo) {
        ...
        if (typeof text === 'string') {
            ...
            if (height < currentLineHeight) {
                throw new Error('The height of the content area (' + sectionName + ') is too small. Minmium ' + currentLineHeight + '. Given ' + height + '.');
            }
    ...
    function addTableToDocument(document, sectionName, sectionInfo) {
        // Get some informations
        var contentFormat = getValue('options.sections.' + sectionName, undefined); // Retruns undefined, if section not exits
...
//        var currentLineHeight = document.currentLineHeight(true);
        ...
        for (var idxTableHdr = 0; idxTableHdr < tableInfo.length; idxTableHdr++) {
            var tableContentHeader = {
                text : tableInfo.name || '',
                width: tableInfo.width || 0,
                height: 0,
                header: true
            };

            // Has Table header?
            if (tableContentHeader.text) {
                tableContentHeader.height = getTextHeight(document, tableContentHeader.name, tableContentHeader.width || 0);
            }

            tableContent.push(tableContentHeader);
        }
        ...
        lxHelpers.forEach(documentData, function(sectionName, sectionData) {
            ...
            lxHelpers.forEach(sectionData, function(entry) {
                ...
                if (entry.text) {
                    ...
                    sectionInfo = addTableToDocument(currentPDFDocument, sectionName, sectionInfo);
                }
