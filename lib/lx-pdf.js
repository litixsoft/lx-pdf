...
module.exports = function (template) {
    ...
    function setUpPage(document, layout) {
        var pageBackgroundOptions = getValue('options.background'),
            multipleImage = getValue('options.multipleImage'),
        ...
        // sets the multiple images
        if (multipleImage && multipleImage.length != 0) {
                for(var p = 0; p < multipleImage.length; p++) {
                    if(getKeyExist('filename', multipleImage[p])) {
                        document.image(getValue('filename', undefined, multipleImage[p]),  getValue('position.left', 0, multipleImage[p]),   getValue('position.top', 0, multipleImage[p]), getValue('imageformat', {}, multipleImage[p]));
                    }
                }
        ...
        if (showFooter === true && pageFooter && pageFooter.data) {
            ...
            pageFooterFormat.format.width = document.page.width - 0;
    ...
    function addTableToDocument(document, sectionData, sectionInfo, newPageCallback) {
        ...
        if (tableWidth > sectionDimension.width) {
            return {done: true, error: {message: 'Table width is greater then section. Given "' + tableWidth + '", Width "' + sectionDimension.width + '".'}};
        }
        ...
        for (var idxRow = 0; idxRow < tableContentData.length; idxRow++) {
            ...
            if (currentContentHeight < 0.5) {
                ...
                // New Dimension
                if (!sectionDimension) {
                    result = {done: true, error: {message: 'WARNING: Section "' + sectionName + '" not found for Page ' + pageIndex + '.'}};
                    break;
                }
